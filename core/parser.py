...
class For(Expr):
    def __init__(self, initializer: Expr, condition: Expr, incrementer: Expr, code: list[Expr], line: int, col: int, TAV:bool=False, name:str="", var_type:Expr=None, global_:bool=False):
        self.condition = condition
        self.line = line
        self.col = col
...
class Parser:
    ...
    def parse_primary(self) -> Expr:
        ...
        # Identifiers
            ...
            # Postfix ++ / --
                ...
                self.advance()
                if self.peek()[0] == TOK_SUB:
                    self.advance()
                    return Decrement(Var(val, line, col), line, col)

            # Pointer declaration (int*, etc.) already handled elsewhere
            return Var(val, line, col)

        # Parenthesized expr or Cast
        elif typ == TOK_LPAR:
            # Lookahead to detect type-cast
            i = self.pos
            next_tok = self.peek()

            # check if next token is a type or signed/unsigned
            if next_tok[0] in [TOK_INT, TOK_BIT, TOK_CHAR, TOK_BOOL, TOK_VOID,
                            TOK_BYTE, TOK_DWORD, TOK_WORD, TOK_QWORD, TOK_LONG,
                            TOK_UNSIGNED, TOK_SIGNED]:
                i += 1
                # consume * (pointer levels)
                while i < len(self.tokens) and self.tokens[i][0] == TOK_MUL:
                    i += 1

                # if after *s, we find ')', this is a cast
                if i < len(self.tokens) and self.tokens[i][0] == TOK_RPAR:
                    type_tok = self.advance()

                    # handle signed/unsigned prefix
                    if type_tok[0] in [TOK_UNSIGNED, TOK_SIGNED]:
                        base_tok = self.advance()
                        base_type = self.tok_to_ast_type(base_tok, None)
                    ...
                    # consume pointer stars for the type
        ...
        # Endline or unexpected
        elif typ == TOK_ENDL:
            return None
        else:
            errorHandler.parser_error(
                f"Unexpected code in expression: '{val}'",
                line=line, col=col, src=self.code, file=self.filename
            )
    ...
    def parse_for(self) -> Expr:
        ...
        return For(init, conditional, increment, body, line, col, tav, name, var_type, global_)
